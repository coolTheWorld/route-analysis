...
6. 高级分析可配置自动车道最大拟合偏差（默认 `0.05 m`）、默认弯道模式和日志级别；上次用的两点连接方式也会记住。
...
1. 左栏自上而下选：**计算方向**（求道路极限 / 校核给定道路）、**场景**（直角转弯 / 直角 R 档直行转 D 档 / 直角 R 档转弯转 D 档 / U 型转弯）、**通行方向**、**档位**、**工况**。后两个复合场景的档位固定 R 档转 D 档，控件置灰。
2. **车辆参数**默认取设置里的车型与净距阈值，页内改动只留在内存，不写回配置。转弯半径不进配置，默认 1.20 m，是经验值，请按车型核实。
3. **求道路极限**只凭车辆参数反求道路尺寸。结果卡写着「一组最小可行尺寸（非唯一）」—— 这是帕累托前沿上的一个点，每一项在其余项取当前值时都已不能再单独缩小，但换一组尺寸组合同样成立。想固定某几项，改用反向校核。
4. **校核给定道路**填入实际尺寸，给出通过 / 临界 / 越界，判定规则与地图页的越界分析同源。极限工况另给各腿的最优偏移和可行偏移区间。
5. **工况**决定车辆能不能偏离道路中心线。极限工况下求解器会顺带给出最优偏移；它只是多给自由度，因此每一项尺寸都不会比中心线工况更大。
...
- **直角转弯**：直线进 → 与两腿相切的四分之一弧 → 直线出。双向为 T 口，跑西进、东进两条镜像机动。
- **直角 R 档直行转 D 档**：R 档沿倒车道直行穿过转出道并继续下探，再换 D 档转出。双向为十字。
- **直角 R 档转弯转 D 档**：车辆起步时整个停在支路里、车头朝支路深处，R 档一段四分之一弧倒转到主路一侧，车头摆正后换 D 档沿主路开走。**支路不设尽头**（车只是停在其中，再深也不构成约束），因此不求支路深度；改为给出**出弯主路深度** —— 倒车摆出占用的那一段主路，自支路开口的外侧壁量起。它由扫掠结果量得而非求解得出，所以结果卡里不加粗。
